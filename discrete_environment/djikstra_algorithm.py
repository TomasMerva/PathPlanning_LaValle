...
def DjikstraAlgorithm(draw, start, goal):
    open_set, came_from = {}, {}
    visited = [start]
    start.cost = 0
    # Initialize Qfunction with the start state
    open_set[start] = start.cost
    ...
    while len(open_set) > 0:                                                    # line 2
        ...
        current_state = min(open_set.items(), key=lambda item: item[1])[0]      # line 3
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_SPACE and start_node and goal_node:
                    ...
                    DjikstraAlgorithm(lambda: world.draw(), start_node, goal_node)
                # reset environment
